...
import HotelEvaluation from "../../components/HotelEvaluation";
...
const RoomDetailsPage = () => {
  ...
  const [endTime, setEndTime] = useState(new Date(new Date().getTime() + 24 * 60 * 60 * 1000)); // Set endTime to one day after startTime
  ...
  const handleStartTimeChange = (date) => {
    setStartTime(date);
    setEndTime(new Date(date.getTime() + 24 * 60 * 60 * 1000)); // Ensure endTime is always one day after startTime
  };

  const handlePayNowClick = () => {
    navigate(`/payment/hotels/${id}/users/${userId}`, {
      state: {
    ...
    <div className="bg-gray-100 min-h-screen">
      ...
      <main className="container mx-auto py-8">
        <div className="bg-white rounded-lg shadow-lg p-6 md:p-10">
          ...
          <HotelEvaluation hotelId={id} userId={userId} />
        </div>
      </main>
    </div>
  );
